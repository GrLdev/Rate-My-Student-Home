...
class CreateReviewForm(FlaskForm):
    ...
    letting_agent = SelectField("Letting Agent", default='none' ,choices=sorted([('none', 'None/Other')] + [(agent.id, agent.name) for agent in EstateAgent.query.all()], key=lambda x: x[1]))
